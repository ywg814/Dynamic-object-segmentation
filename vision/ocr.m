...
%    figure; imshow(Iocr);
%
% See also ocrText, ocrText>locateText, insertShape, insertText
...
if islogical(I) && ~params.PreprocessBinaryImage  
    ...
    Iu8 = im2uint8(I);    
    img = convertRGBToGray(Iu8);
    
    [rawtext, metadata] = tesseract(params, img, roi, hasROI);    
end

txt = ocrText.create(rawtext, metadata);
...
if isCodegen()  
    ...
    [txt, ocrMetadata] = vision.internal.buildable.OCRBuildable.tesseract(tessOpts, Iu8, hasROI);       
else
    tessOpts = parseParams(params);    
    [txt, ocrMetadata] = tesseractWrapper(tessOpts, Iu8, hasROI, roi);     
end
...
if ischar(userLanguage)
    ...
    if ~isCustomLanguage        
        lang = vision.internal.ocr.validateLanguage(modifiedLang, ocrSpkgInstalled());        
    else
    ...
    if all(isCustomLanguage)
        ...
        isSupportPackageInstalled = ocrSpkgInstalled();
...
str = validatestring(layout,{'Auto','Block','Line','Word'},...
...
% -------------------------------------------------------------------------
% Convert RGB to gray for codegen 
% -------------------------------------------------------------------------
function Igray = rgbToGray(I)
persistent converter
if isempty(converter)   
    converter = vision.ColorSpaceConverter('Conversion','RGB to intensity');
end
sz = size(I);
Igray = coder.nullcopy(zeros(sz(1:2),'uint8')); %#ok<NASGU>
Igray = step(converter,I);
...
% -------------------------------------------------------------------------
function tessdata = locateTessdataFolder()
coder.extrinsic('fullfile');
if ocrSpkgInstalled()
    % query which to determine where support package is installed.
    spkgFile = coder.internal.const('vision.internal.ocr.isOCRSupportPackageInstalled');
    if isCodegen()
        fp = vision.internal.codegen.which(spkgFile);
    else
        fp = which(spkgFile);
    end
    
    % determine location of tessdata within the support package
    pattern = coder.internal.const(fullfile('ocrlanguages','toolbox','vision','supportpackages','visionocr')); 
    idx     = strfind(fp, pattern);
    if isempty(idx)
        % required for codegen when support package is not installed
        tessdata = '';
    else           
        last = idx(end); % use the last occurrence of the pattern
        
        % The tessdata folder is installed under ocrlanguages/tesseract-ocr
        tessdata = coder.internal.const(...
            fullfile(fp(1:last+numel('ocrlanguages')),'tesseract-ocr'));       
    end
else
    if isdeployed && ~isCodegen()
