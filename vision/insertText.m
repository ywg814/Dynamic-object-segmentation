...
[RGB, position, text, isScalarText, anchorPoint, textColor, boxColor, ...
    boxOpacity, font, fontFileName, faceIndex, fontSize, isEmpty] = ...
...
if isSimMode
  [glyphStruct,fontStruct,maxBitmapSize]= ...
      populateGlyphBuffer_sim(fontFileName,faceIndex,fontSize);
else
  [glyphStruct,fontStruct,maxBitmapSize]= ...
...
function [RGB,position,outText,isScalarText,anchorPoint,textColor,...
    boxColor,boxOpacity,font,fontFileName,faceIndex,fontSize,isEmpty] = ...
    validateAndParseInputs(I, position, text, varargin)
...
if isSimMode()
  [anchorPoint, textColor, boxColor, boxOpacity, font, fontSize] = ...
...
function [anchorPoint,textColor,boxColor,boxOpacity, ...
    font,fontSize] = validateAndParseOptInputs_sim(inpClass,varargin)
...
%==========================================================================
function [anchorPoint,textColor,boxColor,boxOpacity, ...
    font,fontSize] = validateAndParseOptInputs_cg(inpClass,varargin)
% Validate and parse optional inputs
...
% Validate 'Font'. Do a case insensistive match
...
persistent oldGlyphBitmapArray ...
           oldGlyphIdxFromCharcode oldGlyphBitmapStartIdx ...
           oldGlyphWidths oldGlyphHeights oldGlyphXAdvances ...      
           oldGlyphLeftBearings oldGlyphTopBearings ...
           oldFontAscend oldFontDescend oldFontLinespace ...
           oldMaxBitmapSize

persistent oldFontFileName oldFaceIndex oldFontSize

if isempty(oldFontFileName)
    oldFontFileName = fontFileName;
end
if isempty(oldFaceIndex)
    oldFaceIndex = faceIndex;
end
if isempty(oldFontSize)
    oldFontSize = fontSize;
end

firstTime = isempty(oldGlyphBitmapArray);
changedFontFileName = ~strcmp(oldFontFileName, fontFileName);
changedFontSize = (oldFontSize ~= fontSize);
changedFaceIndex = (oldFaceIndex ~= faceIndex);
glyphInfoChanged = (changedFontFileName || changedFontSize || changedFaceIndex);

if (firstTime || glyphInfoChanged)
    ...
    oldGlyphBitmapArray     = glyphBitmapArray; 
    oldGlyphIdxFromCharcode = glyphIdxFromCharcode;   
    oldGlyphBitmapStartIdx  = glyphBitmapStartIdx; 
    oldGlyphWidths          = glyphWidths;          
    oldGlyphHeights         = glyphHeights;        
    oldGlyphXAdvances       = glyphXAdvances;       
    oldGlyphLeftBearings    = glyphLeftBearings; 
    oldGlyphTopBearings     = glyphTopBearings; 
    %
    oldFontAscend           = fontAscend; 
    oldFontDescend          = fontDescend; 
    oldFontLinespace        = fontLinespace;  
    oldMaxBitmapSize        = maxBitmapSize;
else
    glyphBitmapArray     = oldGlyphBitmapArray; 
    glyphIdxFromCharcode = oldGlyphIdxFromCharcode;   
    glyphBitmapStartIdx  = oldGlyphBitmapStartIdx; 
    glyphWidths          = oldGlyphWidths;          
    glyphHeights         = oldGlyphHeights;        
    glyphXAdvances       = oldGlyphXAdvances;       
    glyphLeftBearings    = oldGlyphLeftBearings; 
    glyphTopBearings     = oldGlyphTopBearings; 
    %
    fontAscend           = oldFontAscend; 
    fontDescend          = oldFontDescend; 
    fontLinespace        = oldFontLinespace;
    %
    maxBitmapSize        = oldMaxBitmapSize;
end
...
function [tbWidth, tbHeight, spaceCharWidth] = ...
    getTextboxWidthHeight(ucTextU16, glyphIdxFromCharcode, ...
                          glyphXAdvances, fontStruct, font)

% New line cracater does not have any width; It just contributes to height
...
if isempty(idxNewlineChar)
   ...
   % for charcater code with no glyph, we replace that by space; account
   % for space width
   numMissingGlyph=sum(uint32(glyphIdxFromCharcode(thisCharcodes_1b))==0);
   hasMissingCharInFont = (numMissingGlyph ~= 0);
...
%==========================================================================
function [tbTopLeftX, tbTopLeftY] = getTextboxTopLeftPostion( ...
    tbLocationXY, tbWidth, tbHeight, anchorPoint)
...
[tbTopLeftX, tbTopLeftY] = getTextboxTopLeftPostion(tbLocationXY, ...
    tbWidth, tbHeight, anchorPoint);
