        ...
        function errors = refine(this, imagePoints, shouldComputeErrors)
            ...
            function reprojectedPoints = reprojectWrapper(paramsVector, ~)
                ...
                for i = 1:numPatterns
                    ...
                    t = paramStruct.translationVectors(i, :)';
                    reprojPointsHomog = worldPointsHomog * (intrinsicMatrix * ...
                        [R(:, 1), R(:, 2), t])';
                    reprojectedPoints(:,:,i) = bsxfun(@rdivide, ...
                        reprojPointsHomog(:, 1:2), reprojPointsHomog(:,3));
                end
                
                % apply distortion
                reprojectedPoints = visionDistortPoints(reprojectedPoints, ...
                    intrinsicMatrix, paramStruct.radialDistortion, ...
                    paramStruct.tangentialDistortion);
                
                reprojectedPoints = ...
                    arrangeImagePointsIntoMatrix(reprojectedPoints);
            end           
