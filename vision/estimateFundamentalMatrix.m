...
%   objects, SURFPoints objects, MSERRegions objects, BRISKPoints objects,
%   or M-by-2 matrices of [x,y] coordinates. matchedPoints1 and
%   matchedPoints2 must contain points which are putatively matched by
%   using a function such as matchFeatures.
%  
%   [F, inliersIndex] =
...
%        Norm8Point, RANSAC, and MSAC require at least 8 points, LMedS 16
%        points, and LTS ceil(800/InlierPercentage).
...
%  
%   'Method'             One of the strings: 'Norm8Point', 'LMedS',
%                        'RANSAC', 'MSAC', or 'LTS'.
...
%                        the Method parameter to 'LMedS', 'RANSAC', 'MSAC',
%                        or 'LTS'. When you set the Method parameter to
%                        either 'LMedS' or 'LTS', the function sets this
%                        parameter to the actual number of trials. When you
%                        set the Method parameter to either 'RANSAC' or
%                        'MSAC', the function sets this parameter to the
%                        maximum number of trials and the actual number of
%                        trials depends on matchedPoints1,
%                        matchedPoints2, and the value of the Confidence
%                        parameter.
% 
%                        Default: 500
%  
...
%                        Specifies the distance threshold for finding
%                        outliers. This parameter applies when you set the
%                        Method parameter to 'RANSAC' or 'MSAC'.
% 
%                        Default: 0.01
% 
...
%
% See also epipolarLine, extractFeatures, matchFeatures,
% cameraPose, estimateUncalibratedRectification,
...
coder.internal.errorIf(status==statusCode.NotEnoughPts, ...
  'vision:estimateFundamentalMatrix:notEnoughPts');

coder.internal.errorIf(status==statusCode.NotEnoughInliers, ...
  'vision:estimateFundamentalMatrix:notEnoughInliers');
...
if nPts >= 8
  ...
  while curNTrials < maxNTrials
    ...
    [curInliers, curNInliers] = findInliers(d, nPts, threshold);
    curDis = sum(d(curInliers)) + ...
      cast((nPts - curNInliers), outputClass) * threshold;
  ...
  if sum(cast(inliers, integerClass), 'native') >= cast(8, integerClass)
    status = statusCode.NoError;
  else
    status = statusCode.NotEnoughInliers;
  end
...
indices = generateRandomIndices(nPts, outputClass, integerClass);
f = norm8Point(pts1h(:, indices), pts2h(:, indices), outputClass,...
  integerClass);
d = computeDistance(disType, pts1h, pts2h, f);

%========================================================================== 
% Generate 8 unique random indices. The algorithm is summarized as follows:
% 1. Initialize 'indices' output to an empty vector and initialize
%    'nSets' to 0.
% 2. Generate a set of 8 random integers ranging from 1 to 'nPts'.
% 3. Find the unique random values and add them to 'indices'.
% 4. If 8 unique values have been added to 'indices', return; otherwise, 
%    do the following:
% 4.1 If 'nSets' is less than 5, generate another set of 8 random integers
%     ranging from 1 to nPts, increment 'nSets', and go to Step 3.
% 4.2 If 'nSets' equals to 5, generate 8 deterministic integers ranging
%     from 1 to nPts and go to Step 3.
%========================================================================== 
function indices = generateRandomIndices(nPts, outputClass, integerClass)

indices = zeros(8, 1, integerClass);
nUnique = zeros(1, integerClass);
nSets = zeros(1, integerClass);
idxIn = ones(1, integerClass);

% Generate a set of 8 random values.
randVal = cast(ceil(rand([1,8], outputClass) * cast(nPts, outputClass)),...
  integerClass);

% Loop until 8 unique numbers have been found.
while nUnique < cast(8, integerClass)
  % Check if the random value is different from all previously selected 
  % ones.
  isUnique = true;
  for idxOut = 1: nUnique
    if randVal(idxIn) == indices(idxOut)
      isUnique = false;
      break;
    end
  end
  
  % If the random value is unique, use it.
  if isUnique
    nUnique = nUnique + ones(1, integerClass);
    indices(nUnique) = randVal(idxIn);
  end
  
  % Move to the next random value; if there are no more random values,
  % generate a new set of random values or use deterministic values.
  if idxIn < cast(8, integerClass)
    idxIn = idxIn + 1;
  else
    if nUnique < cast(8, integerClass)
      % We will try 5 sets of random values. If we cannot find enough
      % random values from 5 sets, we will use any index that has not
      % been used.
      maxNumSetOfRandomDrawing = cast(5, integerClass);
      if nSets < maxNumSetOfRandomDrawing
        randVal = cast(ceil(rand([1, 8], outputClass) *...
          cast(nPts, outputClass)), integerClass);
        nSets = nSets + 1;
      else
        firstVal = randVal(8);
        randVal = firstVal + cast(1:8, integerClass);
        for idx = cast(1, integerClass): cast(8, integerClass)
          if randVal(idx) > nPts
            randVal(idx) = randVal(idx) - nPts;
          end
        end
      end
      idxIn(:) = 1;
...
inliers = coder.nullcopy(false(1, nPts));
nInliers = zeros(1, 'like', nPts);
