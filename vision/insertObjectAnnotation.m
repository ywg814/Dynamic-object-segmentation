...
% insertObjectAnnotation does not work without the JVM.
if ~usejava('jvm')
    error(javachk('jvm'));
end
...
[textInserter, boxInserter, cache] = getSystemObjects(shape, ...
    textBoxOpacity, fontSize, lineWidth, class(I), isLabelNumeric, cache);

tuneLineWidth(boxInserter, lineWidth);
%% == Output ==
% insert shape
...
if isEmpty    
    [lineWidth, outLabel,color,textColor,textBoxOpacity,fontSize,isLabelNumeric] = ...
                                                  deal([],[],[],[],[],[],[]);
else
    %--label--
    checkLabel(label);
    ...
    [lineWidth, color, textColor, textBoxOpacity, fontSize] = ...
...
%==========================================================================
function [lineWidth, color, textColor, textBoxOpacity, fontSize] = ...
                               validateAndParseOptInputs(inpClass,varargin)
...
lastCol = repmat(cast(2, 'like', textLocAndWidth),[size(textLocAndWidth,1) 1]);
...
function [textInserter, boxInserter, cache] = getSystemObjects(shape, ...
    textBoxOpacity, fontSize, lineWidth, inpClass, isLabelNumeric, cache)
...
if isempty(cache.boxInserterObjects{inDTypeIdx, shapeIdx})
    ...
    boxInserter.LineWidth = double(lineWidth);
