...
%#ok<*EMCA>
    ...
    properties (Access = private,Nontunable)
        pColorSpaceConverterCG;
        % use setupImpl method to set properties to System object handle
    end
    ...
    methods(Access = protected)     
        ...
        function setupImpl(obj, ~, varargin)
           if (~isSimMode())
                obj.pColorSpaceConverterCG = vision.ColorSpaceConverter(...
                    'Conversion','RGB to intensity'); % input always uint8            
           end            
        end
        ...
        function releaseImpl(obj)
            if (isSimMode())
                ...
                release(obj.pColorSpaceConverterCG);
    ...
    methods(Access = private, Hidden)
        ...
        %------------------------------------------------------------------
        function grayImage = convertToGrayscale(obj, Iu8)
            if ndims(Iu8) == 3
                if isempty(Iu8)
                    ...
                    if isSimMode()
                        grayImage = step(obj.pColorSpaceConverter,Iu8);
                    else
                        grayImage = step(obj.pColorSpaceConverterCG,Iu8);
                    end
                end
            else
                grayImage = Iu8;
