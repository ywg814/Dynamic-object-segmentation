...
if params.UseParallel
    [assignments, dists, varargout{1:nargout-2}] = assignDataToClustersParallel(features, centers);
else
    [assignments, dists, varargout{1:nargout-2}] = assignDataToClustersSerial(features, centers);
end

%--------------------------------------------------------------------------
function [assignments, dists, varargout] = assignDataToClustersSerial(features, centers)
if isempty(features)
    assignments = [];
    dists       = [];
    if nargout == 3
...
%--------------------------------------------------------------------------
function [assignments, dists, varargout] = assignDataToClustersParallel(features, centers)
...
if isempty(pool)
    [assignments, dists] = assignDataToClustersSerial(features, centers);
else
    ...
    parfor n = 1:size(featuresCube,3)
        ...
        [tassignments, tdists, tisValid] = assignDataToClustersSerial(f, centers);
    ...
    [tassignments, tdists, tisValid] = assignDataToClustersSerial(...
        features(end-remainder+1:end,:), centers);
