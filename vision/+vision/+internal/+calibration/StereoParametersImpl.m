...
classdef StereoParametersImpl <  vision.internal.EnforceScalarHandle
    ...
    methods(Hidden)
        ...
        %------------------------------------------------------------------
        function [Hleft, Hright, Q, xBounds, yBounds] = ...
        ...
        %------------------------------------------------------------------
        function pointCloud = reconstructSceneImpl(this, disparityMap)
      ...
      methods(Access=private)        
        ...
        %
        %------------------------------------------------------------------
        function [xBounds, yBounds] = computeOutputBounds(this, imageSize, ...,
                Hleft, Hright, outputView)
            ...
            if strcmp(outputView, 'full')
                [xBounds, yBounds] = computeOutputBoundsFull( ...
                    outBounds1, outBounds2);
            else % valid
                [xBounds, yBounds] = computeOutputBoundsValid(...
                    outBounds1, outBounds2);
            end            
        end
      end
...
validateattributes(camParams1, ...
    ...
    {}, mfilename, 'cameraParameters1'); %#ok<EMCA>
validateattributes(camParams2, ...
    {'vision.CameraParameters', 'cameraParameters'},...
    {}, mfilename, 'cameraParameters2'); %#ok<EMCA>
...
validateattributes(rectificationParams.H1, {'single', 'double'}, ...
    ...
    'stereoParameters', 'paramStruct.RectificationParams.H1'); %#ok<EMCA>
...
validateattributes(rectificationParams.H2, {'single', 'double'}, ...
    {'2d', 'real', 'nonsparse', 'finite', 'size', [3,3]}, ...
    'stereoParameters', 'paramStruct.RectificationParams.H2'); %#ok<EMCA>
...
validateattributes(rectificationParams.Q, {'single', 'double'}, ...
    ...
    'stereoParameters', 'paramStruct.RectificationParams.Q'); %#ok<EMCA>
...
validateattributes(rectificationParams.XBounds, {'single', 'double'}, ...
    ...
    'stereoParameters', 'paramStruct.RectificationParams.XBounds'); %#ok<EMCA>
...
validateattributes(rectificationParams.YBounds, {'single', 'double'}, ...
    {'vector', 'real', 'nonsparse', 'finite', 'size', [1, 2]}, ...
    'stereoParameters', 'paramStruct.RectificationParams.YBounds'); %#ok<EMCA>
...
validateattributes(rectificationParams.OriginalImageSize, {'single', 'double'}, ...
    ...
    'stereoParameters', 'paramStruct.RectificationParams.OriginalImageSize'); %#ok<EMCA>
...
validatestring(rectificationParams.OutputView, {'full', 'valid'}, ...
    'stereoParameters', 'paramStruct.RectificationParams.OutputView'); %#ok<EMCA>
end
...
%--------------------------------------------------------------
function [xBounds, yBounds] = computeOutputBoundsFull(...
    outBounds1, outBounds2)
...
%--------------------------------------------------------------
function [xBounds, yBounds] = computeOutputBoundsValid(...
    outBounds1, outBounds2)
