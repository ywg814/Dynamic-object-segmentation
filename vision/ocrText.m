...
classdef ocrText
    ...
    methods (Access = private)
        ...
        function this = ocrText(txt,metadata)
            if nargin ~= 0    
    ...
    methods(Access = private)
        ...
        function bboxes = getSpaceBBoxes(this,spaceIdx)
            ...
            for i = 1:numel(wordIdx)
                ...
                w = this.pTextInfo.WordBBox(wordIdx(i)+1,1) - x;
                h = wordBBox(4);
    ...
    methods(Hidden, Static)
        ...
        % -----------------------------------------------------------------
        function textInfo = create(txt,metadata)
            if iscell(txt)
                n = size(txt,1);
                if n > 0
                    textInfo(n,1) = ocrText(txt{end},metadata(end)); %#ok<*EMVDF>
                    for i = 1:n-1
                        textInfo(i) = ocrText(txt{i},metadata(i));
                    end
                else
